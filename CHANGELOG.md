...
- 2026-08-24 — Inicialización del repositorio: estructura de carpetas,
...
- 2026-08-24 — Fase 0 completada: Python 3.12, Terraform 1.15.8 y
...
- 2026-08-24 — Fase 1 completada: generador sintético reproducible
...
- 2026-08-24 — Fase 2 completada: módulo Terraform en `/infra` (44
...
- 2026-08-24 — Datos de Fase 1 cargados también en Cloud SQL
...
- 2026-08-24 — Capa Bronze (`/pipelines/bronze`): extracción Cloud SQL
...
- 2026-08-25 — Capa Silver (`/pipelines/silver` + `/pipelines/dbt_finbank`):
...
- 2026-08-25 — Capa Gold (`/pipelines/dbt_finbank/models/marts`): 8
...
- 2026-08-25 — Fase 4 (Orquestación): 3 Cloud Run Jobs (bronze + transform
...
- 2026-08-25 — Fase 5 (Gobierno): 3 roles IAM reales con service account
...
- 2026-08-25 — Endurecimiento post-entrega: los dos correos personales que
  quedaban fijos en el repo (`alert_notification_email` en
  `environments/dev.tfvars`/`prod.tfvars`, y la cuenta de impersonación de Fase 5
  hardcodeada en `infra/governance.tf`) se movieron a variables de Terraform
  (`impersonator_email` nueva en `variables.tf`) que se pasan vía
  `environments/local.tfvars` — archivo personal, no versionado, con plantilla en
  `local.tfvars.example`. Así cualquiera que clone el repo lo reproduce con su
  propio correo, sin tocar código ni los `.tfvars` compartidos.
- 2026-08-25 — **Bug real encontrado y corregido** en una corrida manual de
  verificación: el Workflow terminó en `FAILED` (`KeyError: key not found: rows`
  en el paso `compute_summary`) a pesar de que Bronze, Silver y Gold habían
  completado los 3 con éxito segundos antes (confirmado con
  `gcloud run jobs executions list`). Causa raíz: el paso `get_metrics` llama a
  `googleapis.bigquery.v2.jobs.query` sin `timeoutMs`; si BigQuery no responde
  de forma síncrona dentro del default (10s), la API devuelve
  `jobComplete: false` sin el campo `rows`, y `compute_summary` asumía que
  siempre estaría presente. Fix: se agregó `timeoutMs: 30000` a la consulta en
  `orchestration/pipeline_workflow.yaml`. Aplicado a la infraestructura real
  (`terraform apply`) y verificado con una segunda corrida completa,
  `SUCCEEDED` en 542s: `PIPELINE_SUCCESS_SUMMARY` real con
  `silver_movimientos=497256`, `gold_fact_transacciones=497256`,
  `gold_fact_cartera=29880`, `quality_alerts_count=2998` — cifras coherentes
  con las corridas previas documentadas en Fase 3/4. Evidencia en
  `docs/evidencia/fase4/rerun-manual-2026-08-25/`.
